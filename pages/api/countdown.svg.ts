...
export default async function handler(req: NextRequest){
  ...
  let content = ''
  if(expired){
    content = `<text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" font-family="${escapeHtml(fontFamily)}" font-size="${titleSize}" fill="#${color}">${escapeHtml(expiredMsg)}</text>`
  } else {
    ...
    units.forEach((u,i)=>{
      ...
      items.push(`
        <g>
          ...
          <text x="${numX}" y="${numY}" dominant-baseline="middle" text-anchor="middle" font-family="${escapeHtml(fontFamily)}" font-size="${fontSize}" fill="#${color}">${pad(u.value)}</text>
          <text x="${numX}" y="${labelY}" dominant-baseline="hanging" text-anchor="middle" font-family="${escapeHtml(fontFamily)}" font-size="${labelSize}" fill="#${color}">${escapeHtml(u.label)}</text>
        </g>
      `)
      if(separator && i < units.length-1 && layout!=='vertical'){
        const sepX = x + unitWidth - 5
        const sepY = height/2
        items.push(`<text x="${sepX}" y="${sepY}" dominant-baseline="middle" text-anchor="middle" font-family="${escapeHtml(fontFamily)}" font-size="${fontSize}" fill="#${color}">:</text>`)
      }
    })

    const titleEl = title ? `<text x="50%" y="${titleSize+10}" dominant-baseline="hanging" text-anchor="middle" font-family="${escapeHtml(fontFamily)}" font-size="${titleSize}" fill="#${color}">${escapeHtml(title)}</text>` : ''

    content = `${titleEl}\n<g transform="translate(0,${title? titleSize+20:20})">${items.join('\n')}</g>`
  }

  // background (gradient or solid)
  const bgEl = gradient ? `<defs><linearGradient id="g1" x1="0%" x2="100%"><stop offset="0%" stop-color="#${gradient.split(',')[0] || 'ffffff'}"/><stop offset="100%" stop-color="#${gradient.split(',')[1] || '000000'}"/></linearGradient></defs><rect width="100%" height="100%" fill="url(#g1)"/>` : `<rect width="100%" height="100%" fill="#${bg}"/>`
